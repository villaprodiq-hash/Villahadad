...
const GalleryHeroWidget: React.FC<GalleryHeroWidgetProps> = ({ items = [], theme = 'amber' }) => {
  ...
  const displaySubtitle = currentItem 
    ? `${currentItem.category} • ${currentItem.date || 'Recent'}`
    : "مجموعة مختارة من أفضل أعمالنا الفوتوغرافية";
  // Pink Theme Helper (Now Studio Dark Rose)
  const pinkColor = '#f43f5e'; // Rose 500
  const showPink = theme === 'pink';

  return (
    <div className="relative w-full h-[280px] rounded-3xl overflow-hidden group shadow-2xl">
        ...
        <div className="space-y-3 max-w-2xl">
           ...
           >
              <span 
                ...
                style={{
                    backgroundColor: showPink ? `${pinkColor}1A` : 'rgba(245, 158, 11, 0.2)', // 1A = 10% opacity
                    borderColor: showPink ? `${pinkColor}33` : 'rgba(245, 158, 11, 0.3)', // 33 = 20% opacity
                    color: showPink ? pinkColor : '#fbbf24'
                }}
              >
