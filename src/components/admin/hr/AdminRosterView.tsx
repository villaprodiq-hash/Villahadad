...
import { 
  Users, Clock, Wifi, WifiOff, MapPin, 
  Activity, MoreVertical, Battery, Phone,
...
const AdminRosterView = () => {
    // Staff Config (Mock Data - In real app, fetch from UserService)
    const initialStaff = [
    ...
    const fetchAttendance = async () => {
        ...
        const merged = initialStaff.map(staff => {
            const record = todayRecords.find(r => r.userName.includes(staff.name) || r.userId === staff.id);
            const isLate = !isSunday && record && record.checkIn && parseInt(record.checkIn.split(':')[0]) > parseInt(staff.shift.split(':')[0]);
    ...
    useEffect(() => {
        fetchAttendance();
        const interval = setInterval(fetchAttendance, 10000); // Live update every 10s
