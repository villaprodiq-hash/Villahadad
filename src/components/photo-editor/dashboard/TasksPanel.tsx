/**
 * 📷 Tasks Panel - Photo Editor Dashboard
 * ✅ PRODUCTION READY - No mock data
 * Shows albums from real bookings only
 */
...
const TasksPanel: React.FC<TasksPanelProps> = ({ isManager = false, bookings = [] }) => {
  ...
  const getStatusIcon = (status?: string) => {
    switch (status) {
      ...
      case 'in_progress':
        return <Circle size={16} className="text-amber-400 fill-amber-400/30" />;
      default:
    ...
    return (
      <div className="bg-[#1a1c22] rounded-2xl border border-white/5 p-6">
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-white font-bold text-sm">📷 ألبومات للتعديل</h3>
        </div>
        <div className="flex flex-col items-center justify-center py-8 text-gray-500">
          <AlertCircle size={32} className="mb-2 opacity-50" />
          <p className="text-sm">لا توجد ألبومات جاهزة للتعديل</p>
          <p className="text-xs mt-1">ستظهر هنا بعد جلسات التصوير</p>
        </div>
      </div>
    );
  }
